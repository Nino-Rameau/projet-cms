...
async function getViewData(params) {
  ...
  const { hostname, pagePath } = resolvedParams;
  const pageSlug = pagePath[pagePath.length - 1];

  const site = await prisma.site.findUnique({ where: { domain: hostname } });
  if (!site) return null;
  ...
  return (
    ...
    >
      ...
      {globalHeaderBlocks.length > 0 && (
        <header>
          {globalHeaderBlocks.map(block => <BlockRenderer key={block.id} block={block} siteSlug={site.slug} isCustomDomain={true} />)}
        </header>
      )}
      {pageBlocks.length === 0 ? (
        ...
        <main lang={site.language || 'fr'} className={`mx-auto space-y-5 ${pageSettings.customClassName || ''}`} style={{ width: '100%', maxWidth: mainMaxWidth }}>
          {pageBlocks.map(block => <BlockRenderer key={block.id} block={block} siteSlug={site.slug} isCustomDomain={true} />)}
        </main>
      )}
      {globalFooterBlocks.length > 0 && (
        <footer>
          {globalFooterBlocks.map(block => <BlockRenderer key={block.id} block={block} siteSlug={site.slug} isCustomDomain={true} />)}
        </footer>
      )}
    </div>
  );
