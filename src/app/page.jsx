  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-pb-background text-pb-foreground [font-family:'Sora',_'Manrope',_sans-serif]">
      <div className="fixed inset-0 pointer-events-none -z-10">
        <div className="absolute -translate-x-1/2 rounded-full -top-32 left-1/2 h-80 w-80 bg-blue-400/20 blur-3xl" />
        <div className="absolute rounded-full top-20 right-8 h-72 w-72 bg-indigo-400/20 blur-3xl" />
        <div className="absolute rounded-full bottom-10 left-8 h-72 w-72 bg-sky-400/20 blur-3xl" />
        <div
      ...
      <main>
        <section className="grid w-full grid-cols-1 gap-12 px-6 pt-16 pb-20 mx-auto max-w-7xl md:grid-cols-2 md:items-center md:pt-24">
          <div className="space-y-8 duration-700 animate-in fade-in slide-in-from-bottom-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              {session ? (
                <Link
                  href="/dashboard"
                  className="py-4 text-base font-bold text-center text-white transition bg-blue-600 shadow-lg rounded-2xl px-7 shadow-blue-900/20 hover:-translate-y-1 hover:bg-blue-700"
                >
                ...
                <button
                  onClick={openAuth}
                  className="py-4 text-base font-bold text-white transition bg-blue-600 shadow-lg rounded-2xl px-7 shadow-blue-900/20 hover:-translate-y-1 hover:bg-blue-700"
                >
              ...
              <Link
                href="/dashboard"
                className="py-4 text-base font-semibold text-center transition border rounded-2xl border-pb-border bg-white/80 px-7 text-slate-800 hover:bg-white"
              >
                Voir des exemples
              </Link>
            </div>
          ...
          <div className="relative duration-1000 animate-in fade-in slide-in-from-bottom-10">
            <div className="rounded-[28px] border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-800/10">
              <div className="flex items-center justify-between px-4 py-3 mb-4 rounded-2xl bg-slate-900 text-slate-200">
                <p className="text-xs tracking-[0.16em]">PAGE PREVIEW</p>
                <p className="text-xs text-blue-300">READY TO PUBLISH</p>
              </div>

              <div className="p-3 space-y-3 border rounded-2xl border-slate-100 bg-slate-50/60">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 text-sm font-semibold text-blue-900 rounded-xl bg-blue-50">Offre claire</div>
                  <div className="p-3 text-sm font-semibold rounded-xl bg-emerald-50 text-emerald-900">Preuve sociale</div>
                </div>
                <div className="p-3 text-sm font-semibold rounded-xl bg-sky-50 text-sky-900">
        ...
        <section className="w-full px-6 pb-20 mx-auto max-w-7xl">
          <div className="grid grid-cols-1 gap-4 p-5 border rounded-3xl border-pb-border bg-white/75 md:grid-cols-2">
            {transformations.map((block) => (
              <div key={block.title} className="p-6 bg-white border rounded-2xl border-slate-200">
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{block.title}</p>
                <ul className="mt-4 space-y-2 text-sm text-slate-700">
                  {block.points.map((point) => (
                    <li key={point} className="flex items-start gap-2">
                      <span className="w-2 h-2 mt-1 rounded-full bg-slate-900" />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
        ...
        <section className="w-full px-6 pb-20 mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {promises.map((feature) => (
              <article
                key={feature.title}
                className="transition border shadow-sm rounded-3xl border-pb-border bg-white/85 p-7 hover:-translate-y-1 hover:shadow-xl"
              >
        ...
        <section className="w-full px-6 pb-20 mx-auto max-w-7xl">
          <div className="rounded-[2rem] border border-pb-border bg-gradient-to-br from-[#111827] via-[#0f172a] to-[#1e293b] p-8 text-slate-100 md:p-12">
            ...
            <div className="grid gap-4 mt-8 md:grid-cols-3">
              {useCases.map((item) => (
                <article key={item.title} className="p-5 border rounded-2xl border-white/20 bg-white/5">
        ...
        <section className="w-full px-6 pb-20 mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {testimonials.map((item) => (
              <article key={item.author} className="border rounded-3xl border-pb-border bg-white/80 p-7">
        ...
        <section className="w-full px-6 pb-20 mx-auto max-w-7xl">
          ...
          <div className="space-y-3">
            {faqs.map((faq) => (
              <details key={faq.question} className="p-5 border group rounded-2xl border-pb-border bg-white/75 open:bg-white">
                <summary className="text-lg font-extrabold list-none cursor-pointer text-slate-900">
                  {faq.question}
                </summary>
        ...
        <section className="w-full px-6 pb-24 mx-auto max-w-7xl">
          <div className="rounded-[2rem] border border-pb-border bg-gradient-to-br from-blue-50 via-white to-sky-50 p-8 text-slate-900 md:p-12">
            ...
            <p className="max-w-2xl mt-4 text-base text-slate-700">
            ...
            <div className="flex flex-col gap-3 mt-8 sm:flex-row">
              ...
              <Link
                href="/dashboard"
                className="py-4 text-base font-semibold text-center transition border rounded-2xl border-slate-300 bg-white/80 px-7 text-slate-800 hover:bg-white"
              >
                Voir mon espace
              </Link>
            </div>
